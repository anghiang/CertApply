...
var getPbftViewCmd = &cobra.Command{
	Use:   "getPbftView",
	Short: "                                   Get the latest PBFT template(PBFT consensus only)",
	Long: `Returns the latest PBFT template in the specified group where the node is located.
The PBFT template is encoded in hex`,
	Args: cobra.NoArgs,
	Run: func(cmd *cobra.Command, args []string) {
		pbft, err := RPC.GetPBFTView(context.Background())
		if err != nil {
			fmt.Printf("PBFT template not found: %v\n", err)
			return
		}
		fmt.Printf("PBFT template: \n%s\n", pbft)
	},
}
