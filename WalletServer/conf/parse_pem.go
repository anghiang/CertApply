...
// LoadECPrivateKeyFromPEM reads file, divides into key and entity
func LoadECPrivateKeyFromPEM(path string) ([]byte, string, error) {
	...
	ecPirvateKey, curve, err := parsePKCS8ECPrivateKey(block.Bytes)
	if err != nil {
		return nil, "", fmt.Errorf("Failure reading private key from \"%s\": %s", path, err)
	}
	return ecPirvateKey, curve, nil
}

// LoadECPublicKeyFromPEM reads file, divides into key and entity
...
func LoadECPrivateKeyFromPEMS(path string) (*ecdsa.PrivateKey, string, error) {
	raw, err := ioutil.ReadFile(path)
	if err != nil {
		return nil, "", err
	}
	...
	ecPirvateKey, err := parsePKCS8ECPrivateKeys(block.Bytes)
	if err != nil {
		return nil, "", fmt.Errorf("Failure reading private key from \"%s\": %s", path, err)
	}
	return ecPirvateKey, "", nil
}
func parsePKCS8ECPrivateKeys(der []byte) (*ecdsa.PrivateKey, error) {
	oidPublicKeyECDSA := asn1.ObjectIdentifier{1, 2, 840, 10045, 2, 1}
	// AlgorithmIdentifier represents the ASN.1 structure of the same name. See RFC
	// 5280, section 4.1.1.2.
	type AlgorithmIdentifier struct {
	...
	d := new(big.Int).SetBytes(privKey.PrivateKey)
